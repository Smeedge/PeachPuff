...
namespace Project1
{
    public class CoinField
    {
        ...
        {
            while (Coins.Count < 20)
            {
                Coin coinObj = new Coin();
                coinObj.CoinModel = coinModel;
                coinObj.position = RandomVector(-266, 266);
                coinObj.size = 10;
        ...
        public Vector3 RandomVector(float min, float max)
        {
            return new Vector3((float)(min + (random.NextDouble() * (max - min))),
                -300, (float)(min + (random.NextDouble() * (max - min))));
        }
        ...
        public void Update(GameTime gameTime)
        {
            ...
            //{
            //    Matrix.CreateRotationY(coin.angle);
            //}
        }
        ...
        public void Draw(GraphicsDeviceManager graphics, GameTime gameTime)
        {
            ...
            {
                DrawModel(graphics, coinModel, Matrix.CreateScale(0.25f) * Matrix.CreateTranslation(butter_fly.position));
            }
        }
        ...
        private void RemoveCoin(Vector3 position)
        {
            for (LinkedListNode<Coin> CoinNode = Coins.First; CoinNode != null; )
            {
                ...
                Coin COIN = CoinNode.Value;

                if (position == COIN.position)
                {
                    // The Coin has been eaten. Remove it and return true.
                    Coin newCoin = new Coin();
                    newCoin.size = COIN.size;
                    newCoin.CoinModel = COIN.CoinModel;
        ...
        public bool TestSphereForCollision(BoundingSphere sphere)
        {
            for (LinkedListNode<Coin> CoinNode = Coins.First; CoinNode != null; )
            {
                ...
                bs.Radius *= COIN.size;
                bs.Center += COIN.position;

                if (sphere.Intersects(bs))
                {
                    System.Diagnostics.Trace.WriteLine("Coin collided!\n");
                    RemoveCoin(COIN.position);
                    return true;
                }
